import type { Bbox, GridCell } from '../types';
...
export function computeGrid(bbox: Bbox, cellKm: number): GridCell[] {
  ...
  for (let lat = bbox.minLat; lat < bbox.maxLat; lat += latStep) {
    for (let lon = bbox.minLon; lon < bbox.maxLon; lon += lonStep) {
      cells.push({
        bounds: [
          [lat, lon],
          [Math.min(lat + latStep, bbox.maxLat), Math.min(lon + lonStep, bbox.maxLon)],
        ],
      });
    }
  }
...
export function estimateBusinesses(count: number): number {
  return count * 15;
}
...
export function estimateMinutes(count: number): number {
  return Math.ceil(estimateSeconds(count) / 60);
}

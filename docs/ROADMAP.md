...
| bug·4 | `0032-batch-analyze-compose-slow-site-resilience` | Sibling of 0031 (clear-lane residual): a genuinely slow site still dead-letters a lead on `analyze_timeout`/`compose_timeout` (4/30 on run 0d4a4131). Two faults — PSI's 60s internal timeout can dominate the 120s analyze budget, and `withTimeout` (Promise.race) dead-letters in-flight work that doesn't stop and finishes `done` moments later. Fix: bound PSI in the batch path; make analyze/compose timeouts recoverable (bounded retries → revert to `pending`, reuse the now-`done` analysis via 0031's F2 guard + TTL; only `failed_slow` after K). Also closes the 0031 F2 contrived-overlap live proof | done | [0032](SLICES/0032-batch-analyze-compose-slow-site-resilience.md) |
